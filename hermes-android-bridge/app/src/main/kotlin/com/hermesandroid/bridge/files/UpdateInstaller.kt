...
/**
 * APK self-update (Phase 4): the agent tells the app to fetch a
...
object UpdateInstaller {
    ...
    fun start(
        ...
        if (android.os.Build.VERSION.SDK_INT >= 26 &&
            ...
            return Result(
                false, 403,
                "Allow 'Install unknown apps' for this app (settings page opened) — then trigger the update again",
            )
        }

        // Installations-Intent (User-Dialog, kein stiller Pfad)
        return try {
            val fileToInstall = File(context.cacheDir, "bridge-update.apk")
