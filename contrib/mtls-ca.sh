#!/usr/bin/env bash
# mTLS CA helper — issue client certificates for device access (see issue #108)
#
...
#
# Nutzung:
#   ./mtls-ca.sh init                     # CA einmalig erzeugen
#   ./mtls-ca.sh issue <gerätename>       # Client-Zertifikat ausstellen
#   ./mtls-ca.sh print <gerätename>       # Fingerprint + Pfade anzeigen
#
...
CA_DIR="${BRIDGE_MTLS_CA_DIR:-/root/mtls-ca}"
CA_KEY="$CA_DIR/bridge-ca.key"
CA_CRT="$CA_DIR/bridge-ca.crt"
...
case "$cmd" in
  init)
    ...
    openssl req -x509 -new -nodes -key "$CA_KEY" -sha256 -days "$CA_DAYS" \
      -subj "/C=AT/O=HermesBridge/CN=HermesBridge Root CA" \
  ...
  issue)
    name="${2:?Usage: mtls-ca.sh issue <gerätename>}"
    ...
    openssl req -new -key "$key" \
      -subj "/C=AT/O=HermesBridge/CN=bridge-$name" -out "$CA_DIR/$name.csr"
...
EOF
    ...
    openssl pkcs12 -export -inkey "$key" -in "$crt" -certfile "$CA_CRT" \
      -name "HermesBridge $name" -passout "pass:$pw" -out "$p12"
  ...
  print)
    name="${2:?Usage: mtls-ca.sh print <gerätename>}"
    crt="$CA_DIR/$name-client.crt"
